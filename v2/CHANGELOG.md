...
- **`docs/architecture/`**: how OpenNMS Horizon 33.1.8 is built inside, in 16 steps with a diagram
  each, using this lab as the running example: the one JVM and how it is loaded, the daemons and
  their Spring contexts, Jetty, the `/opennms` web application, Karaf, Felix, JARs, bundles,
  features and KARs, class loaders and threads, deploying a plugin, the two service registries and
  the bridge between them, how requests reach OSGi, how one server serves plugin A's and plugin B's
  UIs, the shared assets, one page load end to end, and the parts around the core (database, events,
  configuration, logs, time series, security, Minion and Sentinel). The overview maps questions to
  steps and has a glossary.
- The diagrams are SVG files generated by `docs/architecture/images/make_diagrams.py` (standard
  library only); `--check` fails when an SVG is out of date.
- 100 rows in the source trace for the new guide, including the Felix framework 6.0.5 tag.
- `scripts/check_links.py`: checks every relative link and heading anchor in the Markdown files.
- `scripts/validate-repo.sh` checks the diagrams and the links.

### Changed

- `README.md`, `docs/README.md` and the verification page point to the architecture guide.
